...
/* -------------------------- Rahul Code For Woocommerce ---------------------------------------- */

include('primary_walker_nav_menu.php');

/* custom code for woocommerce Rahul */


/* Add Banner to shop page */

/* function my_theme_custom_div()
{ ?>
...
add_action( 'woocommerce_before_main_content', 'my_theme_custom_div', 10 ); */
...
foreach ( $terms as $key => $term ) {
	...
	/* if ( ! in_array( $term->slug, array( 'ladies-formal','ladies-jeans', 'ladies-scuirt', 'ladies-top' ) ) ) {
	ye jo not equal ka sign he(!) usko laga denge to ye char categories ko chod ke baki sab a jayegi category.
		*/
...
add_action('init','wishlis'); */


add_action( 'init', 'wishlistmessgae' );

function wishlistmessgae(){
	?>
	<style type="text/css">
...
/* Add to cart par click karne ke baad checkout page par redirect code */
...
This code is click add to cart after go to checkout directly.
...
function custom_add_to_cart_redirect() {
      ...
      */
	  //echo "rahul";die;
	  //print_r(get_option('woocommerce_checkout_page_id'));die;
...
/* thjis code is single product page after single product summary */
...
//remove_action( 'woocommerce_after_single_product_summary', 'woocommerce_output_related_products', 20 ); sahi he code


add_action( 'custom_func_add_to', 'bbloomer_single_product_type' );
 
function bbloomer_single_product_type() {
global $product;
$link12 = $product->get_permalink();

if( $product->is_type( 'simple' ) ){
 // do something
 ?>
 <a href="<?php echo $link12; ?>" class="btn"><i class="pe-7s-cart"></i></a>
 <?php
} elseif( $product->is_type( 'variable' ) ){
	return sprintf( '<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" data-quantity="%s" class="btn x-btn %s product_type_%s"><i class="pe-7s-cart"></i></a>',
...
function wpdocs_email_friends( $post_id ) {
    $friends = 'sumit@cueserve.com, jaya@cueserve.com, rahul@cueserve.com';
...
add_action( 'wp_head', 'nonrahul' );
function nonrahul(){ ?>

	<form action="<?php echo admin_url( 'options-general.php'); ?>" method="post">
	<input class='button-primary' type='submit' name="foo" value='bar'>
	<?php wp_nonce_field('my_nonce_action','foo'); ?>
	</form>
	
<?php }

	if(isset($_POST['foo'])) {
		if ( !empty($_POST['foo']) && check_admin_referer( 'delete-post', 'foo' )){
			// you're output
		}else{
